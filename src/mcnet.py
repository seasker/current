...
class MCNET(object):
  ...
  def build_model(self):
    ...
    # shape
    cell = BasicConvLSTMCell([ math.ceil(self.image_size[0]/8), math.ceil(self.image_size[1]/8)],
                             [3, 3], 256)
  ...
  def forward(self, diff_in, xt, cell):
    # Initial state
    state = tf.zeros([self.batch_size, math.ceil(self.image_size[0]/8),
                      math.ceil(self.image_size[1]/8), 512])
    reuse = False
    # Encoder
  ...
  def dec_cnn(self, h_comb, res_connect, reuse=False):
    shapel3 = [self.batch_size, math.ceil(self.image_size[0]/4),
               math.ceil(self.image_size[1]/4), self.gf_dim*4]
    shapeout3 = [self.batch_size,math.ceil(self.image_size[0]/4),
                 math.ceil(self.image_size[1]/4), self.gf_dim*2]
    depool3 =max_unpool2d(h_comb,shapel3,[2,2],[2,2])
    ...
    shapel2 = [self.batch_size, math.ceil(self.image_size[0]/2),
               math.ceil(self.image_size[1]/2), self.gf_dim*2]
    shapeout3 = [self.batch_size, math.ceil(self.image_size[0]/2),
                 math.ceil(self.image_size[1]/2), self.gf_dim]
    depool2 = max_unpool2d(deconv3_1,shapel2, [2,2],[2,2])
    ...
    shapel1 = [self.batch_size, self.image_size[0],
               math.ceil(self.image_size[1]), self.gf_dim]
    shapeout1 = [self.batch_size, self.image_size[0],
                 self.image_size[1], self.c_dim]
    depool1 = max_unpool2d(deconv2_1,shapel1, [2,2],[2,2])
    ...
    xtp1 = sigmoid(deconv2d(deconv1_2, output_shape=shapeout1, k_h=3, k_w=3,
